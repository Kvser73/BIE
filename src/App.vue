...
const UNDERWATER_AUTO_ORIGINAL_BLOCK_KEY = 'ext_4'
const comparisonListMarker = {
  key: 'bookmark',
  svg: "<svg style='display:block' xmlns='http://www.w3.org/2000/svg' width='20' height='20' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='1.5' stroke-linecap='round' stroke-linejoin='round'><path d='M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z'/></svg>",
}
const underwaterAutoOriginalSrc = ref('')
const underwaterAutoOriginalLoadSettled = ref(typeof document !== 'undefined' && document.readyState === 'complete')
...
let underwaterAutoOriginalImagePreloader = null
...
function getExtensibilityImageSrc(block) {
  if (!block?.image) {
    return ''
  }
  if (block.key === UNDERWATER_AUTO_ORIGINAL_BLOCK_KEY && underwaterAutoOriginalSrc.value) {
    return underwaterAutoOriginalSrc.value
  }
  return resolveMediaSrc(block.image.src)
}

function clearUnderwaterOriginalPreloader() {
  if (!underwaterAutoOriginalImagePreloader) {
    return
  }
  underwaterAutoOriginalImagePreloader.onload = null
  underwaterAutoOriginalImagePreloader.onerror = null
  underwaterAutoOriginalImagePreloader.src = ''
  underwaterAutoOriginalImagePreloader = null
}

function maybeUpgradeUnderwaterOriginalImage() {
  if (underwaterAutoOriginalUpgradeStarted || heroVideoGateVisible.value || !underwaterAutoOriginalLoadSettled.value) {
    return
  }

  const underwaterBlock = siteContent.sections.extensibility.blocks.find(
    (block) => block.key === UNDERWATER_AUTO_ORIGINAL_BLOCK_KEY,
  )
  const previewSrc = resolveMediaSrc(underwaterBlock?.image?.src)
  const originalSrc = resolvePreviewSrc(underwaterBlock?.image?.previewUrl)

  if (!previewSrc || !originalSrc || previewSrc === originalSrc) {
    return
  }

  underwaterAutoOriginalUpgradeStarted = true
  const preloader = new Image()
  underwaterAutoOriginalImagePreloader = preloader
  ...
  try {
    ...
    // Ignore unsupported image loading hints.
  }

  preloader.onload = () => {
    if (underwaterAutoOriginalImagePreloader !== preloader) {
      return
    }
    underwaterAutoOriginalSrc.value = originalSrc
    clearUnderwaterOriginalPreloader()
  }
  preloader.onerror = () => {
    if (underwaterAutoOriginalImagePreloader !== preloader) {
      return
...
function handleWindowLoadForUnderwaterOriginalUpgrade() {
  underwaterAutoOriginalLoadSettled.value = true
  maybeUpgradeUnderwaterOriginalImage()
}
...
onMounted(() => {
  ...
  window.addEventListener('load', handleWindowLoadForUnderwaterOriginalUpgrade)
...
onBeforeUnmount(() => {
  ...
  window.removeEventListener('load', handleWindowLoadForUnderwaterOriginalUpgrade)
  ...
  <div class="app-root">
    ...
    <div v-if="heroVideoGateVisible" class="hero-gate-overlay">
      <div class="hero-gate-card">
        <p class="hero-gate-title">Loading</p>
        ...
        <section class="shell hero-section">
          ...
          <div ref="heroBulletListRef" class="hero-bullet-list" :style="heroBulletListStyle">
            <div class="arrow-group" v-for="(bullet, index) in siteContent.hero.bullets" :key="`hero-bullet-${index}`">
              <span class="arrow-icon comparison-marker" aria-hidden="true">
                <span class="comparison-marker-glyph" v-html="comparisonListMarker.svg" />
              </span>
      ...
      <section class="shell block-section title-aligned-section balanced-text-section">
        ...
        >
          ...
          >
            ...
            />
            <img
              v-if="getExtensibilityImageSrc(block)"
              :src="getExtensibilityImageSrc(block)"
              :alt="block.image.alt"
              :class="[
                'media-image',
                'progressive-image-final',
        ...
        <div class="tutorial-layout">
          <div class="tutorial-list text-rect">
            <div v-for="(item, index) in siteContent.sections.tutorial.bullets" :key="`tutorial-${index}`" class="tutorial-item">
              <span class="tutorial-marker comparison-marker" aria-hidden="true">
                <span class="comparison-marker-glyph" v-html="comparisonListMarker.svg" />
              </span>
              <p>{{ item }}</p>
            </div>
...
.progressive-image-wrap > .media-image {
  grid-area: 1 / 1;
}
